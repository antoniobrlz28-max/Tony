...
// weighted Wealth .40 / Body .35 / Mind .25. Every component keeps its reason so
// the UI can always answer "why this number". Missing data lowers confidence
// (weights renormalize over available signals) — it is never treated as failure.
...
function scoreWindow(data, asOf) {
  ...
  if (weeklyIncome > 0) {
    ...
    add("wealth", 0.6, "Spending pace", true, lerpScore(ratio, 1.4, 0.7),
      `Spent $${Math.round(spend7)} of ~$${Math.round(weeklyIncome)} weekly income`);
  } else {
    add("wealth", 0.6, "Spending pace", false, 0, "No income logged in the last 4 weeks");
  }

  const cashCount = tx.filter(t => t.type !== "income" && CASH_NOTE.test(t.note || "")).length;
  add("wealth", 0.25, "Cash & Zelle restraint", tx.length > 0, lerpScore(cashCount, 8, 0),
  ...
  add("wealth", 0.15, "Saving action", tx.length > 0, savedThisWeek ? 100 : 30,
  ...
  add("body", 0.35, "Training", logs.length > 0, clamp((trained / 3) * 100),
    `Trained ${trained} of the last 7 days`);

  const drinks = logs.reduce((s, h) => s + (Number(h.alcoholDrinks) || 0), 0);
  add("body", 0.35, "Alcohol", logs.length > 0, lerpScore(drinks, 10, 0),
  ...
  add("body", 0.15, "Fasting", logs.length > 0, clamp((fasts / 2) * 100),
  ...
  if (weightsNow.length && prevLogs.length) {
    ...
    add("body", 0.15, "Weight trend", true, towardGoal ? 100 : (now === prev ? 60 : 30),
      `${delta > 0 ? "+" : ""}${delta} kg vs last week${towardGoal ? ", toward goal" : ""}`);
  } else {
    add("body", 0.15, "Weight trend", false, 0, "Not enough weigh-ins to compare weeks");
  }
  ...
  if (identityScores.length >= 2) {
    ...
    add("mind", 0.5, "Identity score", true, clamp(avgIdentity * 10),
      `Averaging ${avgIdentity.toFixed(1)}/10 across ${identityScores.length} days`);
  } else {
    add("mind", 0.5, "Identity score", false, 0, "Fewer than 2 identity check-ins this week");
  }

  add("mind", 0.35, "Check-in consistency", true, clamp((logs.length / 7) * 100),
  ...
  add("mind", 0.15, "Weekly review", true, reviewFresh ? 100 : 30,
    reviewFresh ? "Weekly review done" : "No weekly review in the last 9 days");

  // ---- Aggregate (renormalize over available signals) ----
  const DOMAIN_WEIGHTS = { wealth: 0.4, body: 0.35, mind: 0.25 };
  const domains = ["wealth", "body", "mind"].map(key => {
    ...
    return { key, label: key === "wealth" ? "Wealth" : key === "body" ? "Body" : "Mind", score: score === null ? null : Math.round(score) };
  });
